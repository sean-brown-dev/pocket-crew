...
Enable vision support in the Pocket Crew app by allowing users to attach images via the `InputBar`, analyzing those images using a dedicated vision model, and feeding the resulting description into the existing Fast, Thinking, or Crew mode pipelines. This must be designed cleanly without bloating the `GenerateChatResponseUseCase`. A vision model is guaranteed to always be selected (either on-device or API).
...
- [ ] Task 5. **Update `ChatViewModel`**
  ...
  - Keep `ChatViewModel` free of Android file/URI handling. Introduce a domain-facing `ImageAttachmentStoragePort` plus a `StageImageAttachmentUseCase` to stage a selected `content://` URI into app-controlled app-private storage.
  - Implement the Android-specific image staging/copy/compression logic in the data layer behind that port, so the presentation layer only deals with a staged URI string.
...
- [ ] Task 7. **Update Inference Service Implementations**
  ...
  - For providers that do not support image inputs today, fail fast with a clear error instead of silently dropping the attachment.
...
- [ ] The `AnalyzeImageUseCase` successfully intercepts the image, calls the guaranteed vision model, and returns a text description.
...
- [ ] Image-only messages remain intelligible after persistence and restore, either by rendering the image in the transcript or by preserving a textual placeholder for history rehydration.

## Potential Risks and Mitigations

1. **URI Permission Expiration**
   Mitigation: a data-layer image staging component will copy the selected image to app-private file storage and use the local `file://` URI for persistence and processing.
2. **Large Image Processing Overhead**
   Mitigation: The image staging utility should downscale or compress images to a reasonable resolution (e.g., max 1024x1024) before saving them to app-private storage to avoid memory issues (OOM) during Base64 conversion.
3. **Silent Capability Mismatch**
   Mitigation: providers without multimodal support must reject image-bearing requests explicitly, so attachments are never ignored without feedback.
4. **History Rehydration Drift**
   Mitigation: preserve image metadata in persisted messages and ensure prompt rehydration includes a stable placeholder or rendered attachment when message text is blank.
