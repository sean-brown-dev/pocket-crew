...
class ImageGenerationPortImpl @Inject constructor(
    ...
    override suspend fun generateImage(
        ...
        return when (provider.provider) {
            ApiProvider.OPENAI -> openAiAdapter.generateImage(prompt, apiKey, provider.modelName ?: "dall-e-3", provider.baseUrl, settings, referenceImageBytes)
            ApiProvider.GOOGLE -> googleAdapter.generateImage(prompt, apiKey, provider.modelName ?: "imagen-3", provider.baseUrl, settings, referenceImageBytes)
            ApiProvider.XAI -> xaiAdapter.generateImage(prompt, apiKey, provider.modelName ?: "grok-vision-beta", provider.baseUrl, settings, referenceImageBytes)
