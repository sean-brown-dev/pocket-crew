...
class EngineModuleTest {
    ...
    fun `model registry returns null for unregistered model types`() {
        ...
        assertNull("DRAFT_ONE should return null when not registered", draftOneConfig)
        assertNull("DRAFT_TWO should return null when not registered", draftTwoConfig)
        assertNotNull("FAST should return config when registered", fastResult)
    }
