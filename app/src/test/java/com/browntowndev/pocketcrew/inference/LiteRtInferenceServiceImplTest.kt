...
class LiteRtInferenceServiceImplTest {
    ...
    @Test
    fun `sendPrompt delegates to use case for thinking token processing`() = runBlocking {
    ...
    @Test
    fun `sendPrompt closes conversation when closeConversation is true`() = runBlocking {
    ...
    @Test
    fun `sendPrompt does not close conversation when closeConversation is false`() = runBlocking {
        val chunks = flowOf("<think>", "thought", "</think>answer")
        every { mockConversation.sendMessageAsync(any()) } returns chunks
