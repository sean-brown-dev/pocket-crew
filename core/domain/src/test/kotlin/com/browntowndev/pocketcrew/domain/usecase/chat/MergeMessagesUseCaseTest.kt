...
class MergeMessagesUseCaseTest {
    ...
    fun `given only db message returns it`() {
        val dbMessage = createMessage(id = 1L, state = MessageState.COMPLETE, content = "db content")
    ...
    fun `given only in-flight returns it`() {
        val inFlightMessage = createMessage(id = 2L, state = MessageState.GENERATING, content = "in-flight content")
    ...
    fun `given both and db is COMPLETE returns db`() {
        val db = createMessage(id = 1L, state = MessageState.COMPLETE, content = "db content")
        val inFlight = createMessage(id = 1L, state = MessageState.GENERATING, content = "in-flight content")
    ...
    fun `given both and db is PROCESSING returns in-flight`() {
        val db = createMessage(id = 1L, state = MessageState.PROCESSING, content = "")
        val inFlight = createMessage(id = 1L, state = MessageState.GENERATING, content = "real content")
        val result = useCase(db, inFlight)
    ...
    fun `given both and db is THINKING returns in-flight`() {
        val db = createMessage(id = 1L, state = MessageState.THINKING, content = "")
        val inFlight = createMessage(id = 1L, state = MessageState.THINKING, content = "thinking content")
        val result = useCase(db, inFlight)
        assertEquals(inFlight, result)
    }
    ...
    fun `given both and db is GENERATING returns in-flight`() {
        val db = createMessage(id = 1L, state = MessageState.GENERATING, content = "partial")
        val inFlight = createMessage(id = 1L, state = MessageState.GENERATING, content = "more content")
        val result = useCase(db, inFlight)
        assertEquals(inFlight, result)
    }
